...
class PennSearch : public PennApplication
{
  public:
    ...
    void ProcessPublish(PennSearchMessage message);
    void ProcessSearchReq(PennSearchMessage message);
    void ProcessSearchRsp(PennSearchMessage message);
    void ProcessFixFinger(PennSearchMessage message);
    void ProcessRspFixFinger(PennSearchMessage message);
    void Publish(std::string word);
    void fix_fingers();
    void FindSuccessor(uint32_t idVal, PennSearchMessage message);
    Ipv4Address ClosestPrecedingNode(uint32_t idVal);
    void ProcessSearchContact(PennSearchMessage message);
    bool InRange(uint32_t id, uint32_t n, uint32_t successor);
    bool IsSuccessor(uint32_t idVal);
    void Search(Ipv4Address destAddress, uint32_t transactionId);
    void PublishPennSearch(Ipv4Address destAddress, uint32_t transactionId);
    void Contact(std::string node, std::vector<std::string> keywords, Ipv4Address originatorAddress);
    ...
    void HandleChordLookupSuccess (Ipv4Address destAddress, PennChord::LookupType type, std::uint32_t transId);
  ...
  private:
    ...
    //finger table
    std::vector<uint32_t> m_fingerTable;
    ...
    //local storage of docs
    std::map<std::string, std::set<std::string>> m_invertedList;

    std::map<uint32_t,  PennSearchMessage> m_searchMessages;

    std::map<uint32_t,  PennSearchMessage> m_publishMessages;
