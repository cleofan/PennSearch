...
public:
  ...
  void ProcessLSP(LSMessage lsMessage, Ipv4Address interface);
  // Periodic Audit
  void AuditPings();
  void BroadcastHello();
  void AuditNeighbors();
  ...
  void LSAdvertisement();
  void Flood(Ptr<Packet> packet, Ipv4Address interface);
...
private:
  ...
  std::map<uint32_t, std::unordered_set<uint32_t>> m_graph;
  std::map<uint32_t, Ipv4Address> m_interfaces;
  std::map<uint32_t, RoutingTableEntry> m_routingTable;
  std::map<Ipv4Address, uint32_t> m_validLSP;
  std::unordered_set<uint32_t> m_neighbors;
