...
public:
  ...
  void ProcessDV(DVMessage dvMessage, Ipv4Address interface);
  void ProcessPathAdv(DVMessage dvMessage, Ipv4Address interface);

  // Periodic Audit
  void AuditPings();
  void BroadcastHello();
  void AuditNeighbors();
  void AuditRoutingTable();
  void AuditRoutingPathTable();
  
  void DVAdvertisement();
  void PathAdvertisement();
...
private:
  ...
  uint8_t m_mainNodeNum;
  Ptr<Ipv4StaticRouting> m_staticRouting;
  Ptr<Ipv4> m_ipv4;
  Time m_pingTimeout;
  Time m_helloTimeout;
  Time m_broadcastHelloTimeout;
  Time m_broadcastAdvTimeout;
  Time m_routingTableTimeout;
  Time m_routingPathTableTimeout;
  Time m_broadcastPathAdvTimeout;
  ...
  Timer m_auditHelloTimer;
  Timer m_broadcastHelloTimer;
  Timer m_broadcastAdvTimer;
  Timer m_routingTableTimer;
  Timer m_broadcastPathAdvTimer;
  Timer m_routingPathTableTimer;
  
  // Ping tracker
  std::map<uint32_t, Ptr<PingRequest>> m_pingTracker;

  std::map<uint32_t, HelloRequest> m_helloTracker;

  std::map<uint32_t, NeighborTableEntry> m_neighborTable;

  std::map<uint32_t, RoutingTableEntry> m_routingTable;

  std::map<uint32_t, RoutingTablePathEntry> m_routingPathTable;

  std::vector<uint32_t> m_distances;

  std::vector<std::vector<uint8_t>> m_paths;
  

  std::map<Ipv4Address, uint32_t> m_validDVP;

  std::map<Ipv4Address, uint32_t> m_validPathDVP;
};
